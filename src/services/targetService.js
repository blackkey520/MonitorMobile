...
import { loadToken, get, getUseNetConfig, upload } from '../logics/rpc';
// 全局api文件
...
export const uploadimage = async (param) => {
  try {
    ...
    const url = `${netconfig.neturl + api.monitortarget.uploadimage}?authorCode=${user.User_ID}`;
...
export const selecttarget = async (param) => {
  try {
    ...
    let result = [];
        // NOTE: 获取网络配置信息
    const netconfig = getUseNetConfig();
    const url = netconfig.neturl + api.monitortarget.targetother;
