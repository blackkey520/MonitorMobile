...
class TargetMonitorPoint extends PureComponent {
        ...
        if (item.NewestRealTimeData && item.NewestRealTimeData != null) {
          ...
          rtnDataVal.push(
            <View key={key} style={{ marginTop: 5, flex: 1, flexDirection: 'row', alignItems: 'center', width: SCREEN_WIDTH - 45, marginLeft: 10, justifyContent: 'space-around', }}>
              ...
              <Text style={{ width: SCREEN_WIDTH / 4 - 10, fontSize: 13, color }}>{item.NewestRealTimeData.Value}</Text>
  ...
  renderMonitorPoints=() => {
    ...
    this.props.targetBase.OtherInfo.map((item, key) => {
      ...
      rtnVal.push(
        <View key={item.Dgimn} style={{ width: SCREEN_WIDTH - 30, marginLeft: 15, marginTop: 15, borderColor: '#e2e1e1', borderWidth: 1, borderRadius: 10, }}>
          <TouchableOpacity
            onPress={() => {
          ...
          <View style={{ marginTop: 5, marginBottom: 10 }}>
            ...
            {
              this.renderMonitorData(item.MonitorPointPollutant)
            }
          </View>
        </View>
